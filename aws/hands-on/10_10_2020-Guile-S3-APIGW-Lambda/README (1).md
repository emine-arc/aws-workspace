...
- Create a bucket of `emine.source.lambda` with following properties,

```text
Bucket name                 : emine.source.lambda
Region                      : N.Virginia
...
- Create another bucket of `emine.destination.lambda` with following properties,

```text
Bucket name                 : emine.destination.lambda
Region                      : N.Virginia
Block all public access     : Checked (KEEP BlOCKED)
Versioning                  : Disabled
Tagging                     : 0 Tags
...
def lambda_handler(event, context):
   destination_bucket_name = 'emine.destination.lambda'
...
https://openexchangerates.org/(for example-API gateway)