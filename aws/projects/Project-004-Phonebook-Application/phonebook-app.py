...
app.config['MYSQL_DATABASE_PASSWORD'] = 'Emine_1'
...
def insert_person(name, number):
    ...
    row = cursor.fetchone() #fetchall degilde fetchone olmasi gerekir
...
@app.route('/', methods=['GET', 'POST'])  #baslangic sayfasi
def find_records():
    if request.method == 'POST':  # girilen method post ise calistir
        ...
        return render_template('index.html', persons_html=persons_app, keyword=keyword, show_result=True, developer_name='Emine')
    else:
...
def add_record():
    if request.method == 'POST':
        ...
        return render_template('add-update.html', show_result=True, result_html=result_app, not_valid=False, action_name='save', developer_name='Emine') #In addition, There is no message shown by user here. Thats why not valid is going to be False.
    else:
...
def update_record():
    if request.method == 'POST':
        ...
        return render_template('add-update.html', show_result=True, result_html=result_app, not_valid=False, action_name='update', developer_name='Emine') #Again, There is no message shown by user here. Thats why not valid is going to be False.
    else:
...
def delete_record():
    if request.method == 'POST':
        ...
        return render_template('delete.html', show_result=True, result_html=result_app, not_valid=False, developer_name='Emine') # In addition, There will be no message to be shown to the user here. Thats why not valid is going to be False.
    else:
